...
Production-ready Go service that listens for Redis `weekly_reco` events, fetches active users, enriches each user with weekly analytics, generates a one-sentence Russian recommendation through a configurable OpenAI-compatible LLM endpoint, and publishes it to Redis `send_message`.
...
- Shared LLM request rate limiter.
- Retry with exponential backoff for retryable LLM HTTP statuses.
...
| `LLM_ENDPOINT` | no | `https://openrouter.ai/api/v1/chat/completions` | OpenAI-compatible chat completions endpoint |
| `OPENROUTER_API_KEY` | yes | - | API key for `LLM_ENDPOINT` |
| `LLM_MODEL` | no | `anthropic/claude-haiku-4.5` | LLM model name |
| `LLM_TIMEOUT` | no | `20s` | Per LLM request timeout |
...
5. Generate one short Russian recommendation through the configured LLM endpoint.
6. Publish to Redis channel `send_message` as:
...
Logs are JSON and include `service_name=recommender`. The service never logs API keys, authorization headers, full prompts, or full LLM responses at INFO level. LLM success logs contain only safe fields: `chat_id`, response length, model name, and latency.
...
Tests use mocks and local HTTP/Redis test doubles. They do not call the configured LLM endpoint.
